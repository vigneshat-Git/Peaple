...
const RegisterPage = () => {
  ...
  const validateForm = () => {
    ...
    else if (!/(?=.*[a-z])(?=.*[A-Z])(?=.*\d)/.test(password)) newErrors.password = "Needs uppercase, lowercase, and number";
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast({ title: "Account created!", description: "Welcome to Peaple." });
      navigate("/");
    } catch (error) {
      ...
      <div className="w-full max-w-sm">
        <div className="text-center mb-6">
          <Link to="/" className="inline-flex items-center gap-2 mb-3">
            ...
            <span className="text-xl font-bold text-foreground">Peaple</span>
          </Link>
          <h1 className="text-xl font-bold text-foreground">Sign Up</h1>
          <p className="text-sm text-muted-foreground mt-1">Join the Peaple community</p>
        </div>
        ...
        <div className="bg-card rounded-md p-6 border">
          ...
          <Button type="button" variant="outline" className="w-full" onClick={handleGoogleSignup} disabled={isGoogleLoading}>
            {isGoogleLoading ? <><Loader2 className="mr-2 h-4 w-4 animate-spin" />Connecting...</> : (
              <>
